...
const OWL_CAT = ['🦉', '🐱'];
const PHOTO = {
  fall: { image: 'img/fall-kecil.jpg', aspect: '3 / 4', caption: 'Fall Jr 🦉' },
  aidan: { image: 'img/aidan-kecil.jpg', aspect: '3 / 4', caption: 'Aidan Jr 🐱' },
  aidan2: { image: 'img/aidan-kecil-2.jpg', aspect: '3 / 4', caption: 'Aidan Jr 🎸' },
  berdua: { image: 'img/berdua.jpg', aspect: '3 / 2', caption: 'Fall Jr 🦉💖🐱 Aidan Jr' },
};
// Muka buat game: yang terbang Fall Jr, yang lari & jadi target lempar Aidan Jr
const FACE = { fall: 'img/fall-head.jpg', aidan: 'img/aidan-head.jpg' };

// Parameter game baru per tingkat (wi = dunia 0–4)
const G = {
  stack: (wi) => ({ type: 'stack', target: [6, 8, 9, 10, 10][wi], speed: 170 + wi * 25, speedUp: 6, lives: 3 }),
  runner: (wi, obstacles) => ({ type: 'runner', time: [20, 24, 26, 28, 28][wi], speed: 230 + wi * 20, gapMin: 1.05 - wi * 0.07, gapMax: 1.8 - wi * 0.1, lives: 3, obstacles, face: FACE.aidan, emoji: '🐱' }),
  throw: (wi) => ({ type: 'throw', target: [5, 6, 7, 7, 7][wi], throws: [10, 11, 11, 11, 11][wi], speed: 120 + wi * 25, speedUp: 10, jitter: 0.2 + wi * 0.1, face: FACE.aidan, emoji: '😘' }),
  maze: (wi) => ({ type: 'maze', cols: [5, 6, 7, 7, 8][wi], rows: [7, 8, 9, 10, 11][wi], time: [45, 50, 55, 60, 60][wi], startFace: FACE.fall, endFace: FACE.aidan }),
  // Panah Cinta sengaja dibuat santai: zona nggak pernah lebih kecil dari 20% bar, jarum nambah cepat sedikit aja
...
const LETTER_LEVEL = 'w3-5'; // kuis terakhir dunia 4 → buka surat
...
const popParams = (wi, d) => ({
  ...
  badRate: 0.15 + d * 0.04, multi: d * 0.12, good: OWL_CAT, bad: ['🐝'], gold: '💖',
});
...
// Pastikan tiap dunia 12 game beda (cuma peringatan buat developer)
WORLDS.forEach((w, wi) => {
  const types = LEVELS.filter((L) => L.world === wi && !L.bonus).map((L) => L.type);
  if (new Set(types).size !== types.length) console.warn(`Dunia ${wi + 1} ada game yang dobel:`, types);
});
let num = 0;
...
function hintFor(L) {
  ...
  switch (L.type) {
    ...
    case 'simon': return `Perhatiin urutan 🦉🐱💖⭐ yang nyala, terus ulangi! Target ${p.target} urutan.`;
    case 'fly': return `Tap buat bikin Fall Jr terbang, lewati ${p.target} tiang bunga & ambil 💖!`;
    case 'stack': return `Tap pas lapisan kuenya di atas kue sebelumnya! Susun ${p.target} tingkat. Pas banget = Perfect ✨`;
    case 'runner': return `Aidan Jr lari! Tap buat lompat (bisa 2x), hindari ${p.obstacles.join('')} selama ${p.time} detik.`;
    case 'throw': return `Lempar 💖 ke Aidan Jr yang gerak-gerak! Kena ${p.target} kali, lemparan cuma ${p.throws}.`;
    case 'maze': return `Tarik garis dari muka Fall Jr lewat jalan labirin sampai ketemu Aidan Jr! Waktunya ${p.time} detik.`;
...
function loadProgress() {
  ...
  // Pindahkan progress lama (24 level pertama) supaya bintang Fall nggak hilang
  const migrated = { stars: {} };
  try {
    const old = JSON.parse(localStorage.getItem('falicya-quest-v1'));
    if (old && Array.isArray(old.stars)) {
      old.stars.forEach((n, i) => { if (n) migrated.stars[`w${Math.floor(i / 6)}-${i % 6}`] = n; });
    }
  } catch {}
  return migrated;
}
...
function renderMap() {
  ...
  const letterBtn = `
    <button class="letter-btn ${letterOpen() ? '' : 'locked'}" id="btn-letter" ${letterOpen() ? '' : 'disabled'}>
      <span>💌</span>
      <div><b>Surat untuk ${esc(CONFIG.name)}</b><small>Bisa dibuka kapan aja 🥰</small></div>
    </button>`;
...
function showResult(L, r) {
  ...
  modal.querySelector('.modal-card').innerHTML = r.win ? `
      <div class="owl-react happy"><span class="owl">🦉</span><span class="owl-extra">${isLast ? '👑' : '🎉'}</span></div>
      <p class="owl-line">${esc(pick(OWL_WIN))}</p>
      <h2>${levelLabel(L)} selesai!</h2>
      <div class="stars">${stars}</div>
      <p class="detail">${esc(r.detail)}</p>
      ${msg ? `<div class="love-note"><span class="from">💌 dari ${esc(CONFIG.from)}</span>${esc(msg)}</div>` : ''}
      <div class="modal-actions">
        <button class="btn ghost" data-go="map">🗺️ Peta</button>
        <button class="btn ghost" data-act="retry">🔁 Ulangi</button>
        ${nextBtn}
      </div>` : `
      <div class="owl-react sad"><span class="owl">🦉</span><span class="owl-extra">💧</span></div>
      <p class="owl-line">${esc(pick(OWL_LOSE))}</p>
...
const ICON = { fall: '🦉', aidan: '🐱' };
const streakSlots = [$('#streak-home'), $('#streak-map')];
let streakData = null;
let streakError = false;

function streakMessage(me, d) {
  const other = me === 'fall' ? 'aidan' : 'fall';
  if (d.litToday) return `Streak hari ini udah nyala! Besok main lagi yaa 💖`;
  if (d.today[me]) return `${me === 'fall' ? 'Fall' : 'Kamu'} udah main hari ini ✓ tinggal nunggu ${NAME[other]} 🥺`;
  if (d.today[other]) return `${NAME[other]} udah main hari ini, sekarang giliran ${me === 'fall' ? 'Fall' : 'kamu'}! 💪`;
...
function streakCard() {
  ...
  const dot = (w) => (w.fall && w.aidan ? '🔥' : w.fall ? '🦉' : w.aidan ? '🐱' : '');
  return `<div class="streak-card ${d.litToday ? 'lit' : ''}">
    <div class="streak-top">
      ...
      <div class="streak-who">
        ${['fall', 'aidan'].map((p) => `<span class="who ${d.today[p] ? 'ok' : ''}">${ICON[p]} ${NAME[p]} ${d.today[p] ? '✓' : '⏳'}</span>`).join('')}
      </div>
    </div>
...
async function refreshStreak(justPlayed = false) {
  ...
  if (justPlayed && streakData) {
    if (streakData.litToday && !before?.litToday) {
      ...
      const other = getPlayer() === 'fall' ? 'aidan' : 'fall';
...
document.addEventListener('click', (e) => {
  ...
  if (e.target.closest('[data-who-reset]')) {
    sfx('click');
    try { localStorage.removeItem('fq-player'); } catch {}
    renderStreak();
  }
});
