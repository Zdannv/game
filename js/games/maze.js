...
export function startMaze(stage, p, api) {
  ...
  const fallFace = loadImage(p.startFace);
  const aidanFace = loadImage(p.endFace);
  const { cols, rows } = p;
  ...
  function draw(now) {
    ...
    // muka Aidan Jr di tujuan (goyang pelan manggil-manggil)
    ...
    drawFace(ctx, aidanFace, g.x, g.y, r * pulse, '🐱', '#ffd1e3');

    // muka Fall Jr di ujung garis
    const h = center(L, path[path.length - 1]);
    drawFace(ctx, fallFace, h.x - meet * 2, h.y, r * (won ? 1.15 : 1), '🦉', '#fff');
  ...
  function end(ok) {
    ...
    api.finish({ win: ok, stars, detail: ok ? `Fall ketemu Aidan, sisa ${Math.ceil(timeLeft)} detik 💞` : 'Waktunya habis sebelum ketemu 🥺' });
  }
